(ns hard.core
  ...
  (:import [
    ...
    Application Color Input Screen Gizmos]))
...
(defmacro the [& terms]
  `(find-name ~(apply str (interpose " " terms))))

(defmacro every [& terms]
  `(arcadia.core/objects-named
    ~(or (first (filter #(instance? System.Text.RegularExpressions.Regex %) terms))
...
(defn delta-time [] UnityEngine.Time/deltaTime)
...
(defn- vget [col idx nf] (or (get col idx) nf))
...
(defn- operate [op -a -b]
  (let [c (max (-count -a)(-count -b))
        ...
        b (if (number? -b) (vec (take c (repeat -b)))
              (-vec -b))]
    (map #(op (vget a % 0) (vget b % 0)) (range c))))
...
(defn ->v3 
  ...
  ([o] (? 
    ...
    (sequential? o) (Vector3. (vget o 0 0) (vget o 1 0) (vget o 2 0))
...
(defn clone!
  ...
  ([ref pos]
    (let [source (? 
            (string? ref) (find-name ref)
            (keyword? ref) 
            (resource (clojure.string/replace (subs (str ref) 1) #"[:]" "/")) 
            :else ref)
          pos  (if pos (->v3 pos) (->v3 source))
          quat  (.rotation (.transform source))
          o (. GameObject (Instantiate source pos quat))]
      (set! (.name o) (.name source))
      (swap! CLONED #(cons o %)) o)))

(defn data! [o v] (swap! _DATA_ conj {o v}) o)

(defn update-data! 
  ([o f] (swap! _DATA_ update o f) o)
  ([o f & xs] (swap! _DATA_ update o #(apply f (cons % xs))) o))
...
(defn color 
  ...
  ([r g b a] 
    (Color. 
      (color-normalized-number r) 
      (color-normalized-number g) 
      (color-normalized-number b) 
      (color-normalized-number a))))
