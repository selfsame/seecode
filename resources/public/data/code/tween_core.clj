...
(deftype Tween [^System.MonoType component opts callbacks]
  ...
  clojure.lang.IObj
  (ToString [this] 
    (str "" component "|" (:duration opts)))
  (meta [this] (meta opts))

  clojure.lang.ILookup
  (valAt [_ k] 
    (if (= k :callback) @callbacks (get opts k)))
  (valAt [_ k nf] 
    (if (= k :callback) @callbacks (get opts k nf)))
  ...
  (equiv [self o]
    (if (instance? Tween o)
      (and (= component (.component o))
         (= opts (.opts o))
         (= @callbacks @(.callbacks o))) false))

  tween.protocols.Chainable
  (-link! [a b] 
    (when (callable? b)
          (swap! (.callbacks a) #(vec (conj (set %) b)))) b)
  (-unlink! [a b]
  ...
  (invoke [this go] 
    (let [pre-c (.GetComponent go component) 
      ...
      (when-not (= (.easesig c) [(:in opts) (:out opts)]) 
        ...
        (set! (.easefn c) (get easeregistry [(:in opts)(:out opts)]))) go)))

(defn callable? [x] (or (instance? Tween x) (fn? x)))
